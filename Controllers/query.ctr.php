...
Class Query{
  ...
  function updatecustomer($table, $customer_name, $customer_phone, $customer_address, $updateid){
    global $pdo;
    $stmt = $pdo->prepare("UPDATE $table SET customer_name='$customer_name', customer_phone='$customer_phone', customer_address='$customer_address' WHERE customer_id='$updateid'");
    $stmt->execute();
  ...
  function updatecoldstore($indate, $outdate, $commondity_id, $mc, $kg, $coldstorerate, $labourrate, $processingrate, $upprocessingcharges, $updateid){
    global $pdo;
    ...
    if(!str_contains(strtolower($comname), 'block')){
      if(!empty($processing)){
        ...
        $pcharges = intval($processingrate) * floatval($kg);
    ...
    if(!empty($stock)){
      $stotal_mc = $stock['total_mc'] - intval($mc);
  ...
  function addfishcharges($date, $ite, $mc, $kg, $coldstorerate, $labourrate, $damagekg){
    ...
    if($ite == 'balance'){
      ...
      if($ite == 'import'){
        ...
        $fishcoldstorestmt = $pdo->prepare("SELECT * FROM gfcfishcoldstore WHERE id < $id ORDER BY id DESC");
        $fishcoldstorestmt->execute();
        $fishcoldstore = $fishcoldstorestmt->fetch(PDO::FETCH_ASSOC);
        $importrowstmt = $pdo->prepare("SELECT * FROM gfcfishcoldstore WHERE date!='$date' AND id < $id ORDER BY id DESC");
        $importrowstmt->execute();
  ...
  function addtotal($date, $plugoncharges){
    ...
    $fishcoldstorestmt = $pdo->prepare("SELECT total_charges FROM gfcfishcoldstore WHERE date='$date'");
    $fishcoldstorestmt->execute();
    $fishcoldstoredata = $fishcoldstorestmt->fetch(PDO::FETCH_ASSOC);

    $fishlabourstmt = $pdo->prepare("SELECT total_charges FROM gfcfishlabour WHERE date='$date'");
    $fishlabourstmt->execute();
    $fishlabourdata = $fishlabourstmt->fetch(PDO::FETCH_ASSOC);

    $repackingstmt = $pdo->prepare("SELECT total_charges FROM repackingout WHERE date='$date'");
  ...
  function addpayment($payment_date, $payment_amount, $id){
    ...
    $balance = intval($totalchargesdata['balance_amount']) - intval($payment_amount);
  ...
  function updateselingrate($selling_rate, $id, $size, $item_id){
    ...
    echo $total_kg = floatval($sizedata['total_kg']);
  ...
  function delaccepttransaction($date, $searchvoucher_no){
    ...
    // Receivable
    $stmt = $pdo->prepare("SELECT * FROM transaction WHERE voucher_no='$searchvoucher_no' AND ac_code LIKE '3300%'");
    $stmt->execute();
    $receivabledatas = $stmt->fetchall();
    foreach ($receivabledatas as $receivabledata) {
      $ac_code = $receivabledata['ac_code'];
    ...
    // Payable
    $stmt = $pdo->prepare("SELECT * FROM transaction WHERE voucher_no='$searchvoucher_no' AND ac_code LIKE '4000%'");
    $stmt->execute();
    $payabledatas = $stmt->fetchall();
    foreach ($payabledatas as $payabledata) {
      $supplier_id = $payabledata['ac_code'];
    ...
    // Cash Book
    $stmt = $pdo->prepare("SELECT * FROM transaction WHERE voucher_no='$searchvoucher_no' AND ac_code LIKE '3600%'");
    $stmt->execute();
    $cashbookdatas = $stmt->fetchall();
    foreach ($cashbookdatas as $cashbookdata) {
      $ac_name = $cashbookdata['ac_code'];
  ...
  function deloldtransaction($voucher_no){
    ...
    $general_ledgerstmt = $pdo->prepare("SELECT * FROM general_ledger WHERE voucherno='$voucher_no'");
    ...
    foreach ($general_ledgerdatas as $general_ledgerdata) {
      if($general_ledgerdata['debit'] != 0){
        $generalledgerstmt = $pdo->prepare("DELETE FROM general_ledger WHERE voucherno='$voucher_no' AND debit!=0");
        $generalledgerstmt->execute();
      }elseif($general_ledgerdata['credit'] != 0){
        $generalledgerstmt = $pdo->prepare("DELETE FROM general_ledger WHERE voucherno='$voucher_no' AND credit!=0");
        $generalledgerstmt->execute();
      }
    }

    // DELETE RECEIVABLE
    $receivablestmt = $pdo->prepare("DELETE FROM receivable WHERE payment_no='$voucher_no'");
    ...
    // DELETE CASHBOOK
    $cashbookstmt = $pdo->prepare("DELETE FROM cashbook WHERE voucher_no='$voucher_no'");
    $cashbookstmt->execute();
  }
