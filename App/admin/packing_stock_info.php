...
<body>
  ...
  if (isset($_POST['updatebtn'])) {
    ...
    $itemstmt = $pdo->prepare("SELECT * FROM products WHERE id='$commondity_id'");
    $itemstmt->execute();
    $itemdata = $itemstmt->fetch(PDO::FETCH_ASSOC);
    $item = $itemdata['name'] ?? '';

    if (!empty($item) && str_contains(strtolower($item), 'block')) {
            ...
            <form action="" method="post">
              ...
              <select class="form-control float-end me-2 inpv2" style="width: 16%; height:28px; padding-top:3px;" name="searchcommondity">
                ...
                foreach ($commonditydatas as $commonditydata) {
                  // FIXED: Products Table
                  $commondityname = $query->select('products', $commonditydata['commondity_id'], 'id');
                  if ($commondityname) {
                ?>
                    <option value="<?= htmlspecialchars($commonditydata['commondity_id']); ?>"><?= htmlspecialchars($commondityname['name']); ?></option>
                <?php
                  }
                } ?>
              </select>
          ...
          ?>
              <table class="table table-striped table-hover table-bordered" id="packingstocktable">
                ...
                ?>
                    <tr>
                      ...
                      <td><?php if (empty($checklastavaliable)) {
                            echo htmlspecialchars($packingstockinfodata['size'] ?? '');
                          } ?></td>
                      ...
                      <td <?php if (!empty($commonditydata['name']) && str_contains(strtolower($commonditydata['name']), 'block')) {
                            echo 'data-bs-toggle="modal"';
                          } ?> data-bs-target="#totalnetweightmodal<?= $packingstockinfodata['id']; ?>"><?php echo $packingstockinfodata['totalgrossweight']; ?></td>
                    </tr>
                    <?php
                    ?>
                ...
                <tr>
                  ...
                  <td style="font-weight:bold !important;"><?php if (!empty($totalnetweightdata['totalnetweight'])) {
                                                              echo $totalnetweightdata['totalnetweight'];
                                                            }; ?></td>
              ...
              <table class="table table-striped table-hover table-bordered" id="packingstocktable">
                ...
                ?>
                    <tr>
                      ...
                      <td><?php echo $packingstockinfodata['mc']; ?></td>
                      <td><?php echo $packingstockinfodata['totalnetweight']; ?></td>
                      <td <?php if (!empty($commonditydata['name']) && (str_contains(strtolower($commonditydata['name']), 'block') || str_contains(strtolower($commonditydata['name']), 'bala'))) {
                            echo 'data-bs-toggle="modal"';
                          } ?> data-bs-target="#totalnetweightmodal<?= $packingstockinfodata['id']; ?>"><?php echo $packingstockinfodata['totalgrossweight']; ?></td>
                      <td>
                          ...
                          <form action="" method="post">
                            ...
                            <div class="modal-body">
                              <div class="row">
                                <div class="col">
                                  ...
                                  <select class="form-control inpv2 mb-2" name="upitem_id">
                                    ...
                                    foreach ($upcommonditydatas as $upcommonditydata) {
                                      ...
                                      if ($commonditydata) {
                                    ?>
                                        <option value="<?php echo htmlspecialchars($commonditydata['id']); ?>" <?php if ($packingstockinfodata['commondity_id'] == $commonditydata['id']) {
                                                                                                                  echo 'selected';
                                ...
                                <div class="col">
                                  <label>Size</label>
                                  <input type="text" name="upsize" class="form-control inpv2 mb-2" value="<?= htmlspecialchars($packingstockinfodata['size']); ?>">
                                </div>
                              </div>
                              <div class="row">
                                <div class="col">
                ...
                ?>
                <?php
                $item_id = $packingstockinfodata['commondity_id'] ?? 0;
                ...
                <tr>
                  ...
                  <td></td>
                  <td></td>
                  <td style="font-weight:bold !important;"><?php echo $totalmcdata['totalmc'] ?? 0; ?></td>
                  <td style="font-weight:bold !important;"><?php if (!empty($totalnetweightdata['totalnetweight'])) {
                                                              echo $totalnetweightdata['totalnetweight'];
                                                            }; ?></td>
                  <td style="font-weight:bold !important;"><?php if (!empty($totalgrssweightdata['totalgrossweight'])) {
            ...
            ?>
              <table class="table table-striped table-hover table-bordered">
                ...
                <tr>
                  ...
                  <td></td>
                  <td></td>
                  <td><?php echo $totalusddata['total_usd'] ?? 0; ?></td>
                </tr>
              </table>
            <?php
            } else {
              ...
              <table class="table table-striped table-hover table-bordered">
                ...
                ?>
                    <tr>
                      ...
                      <td <?php if (!empty($commonditydata['name']) && str_contains(strtolower($commonditydata['name']), 'bala')) {
                            echo 'data-bs-toggle="modal"';
    ...
    $(document).ready(function() {
      ...
      if (isset($_SESSION['tabs']) && $_SESSION['tabs'] == 'actualinvoice') {
        echo '
         ...
         ';
      } elseif (isset($_SESSION['tabs']) && $_SESSION['tabs'] == 'actualpackinglist') {
        echo '
         $("#actualinvoice").show();
