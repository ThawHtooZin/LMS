...
// OPTIMIZATION: Fetch all products once to avoid N+1 queries in loops
$prodStmt = $pdo->prepare("SELECT id, name FROM products");
$prodStmt->execute();
$allProducts = [];
      ...
      <div class="card">
        <form action="" method="post">
          <div class="card-header bg-success">
            ...
            <select name="fish_type" class="form-control d-inline float-end inpv2" style="height:26px; width: 100px; padding-left:10px; padding-top:2px;">
              ...
              ");
              $searchfish_typestmt->execute([':country' => $country]);
              $searchfish_typedatas = $searchfish_typestmt->fetchall();
              foreach ($searchfish_typedatas as $searchfish_typedata) {
              ?>
                <option value="<?php echo htmlspecialchars($searchfish_typedata['fish_type']); ?>"><?php echo htmlspecialchars($searchfish_typedata['fish_type']); ?></option>
              <?php
              }
              ?>
            </select>
            <select class="form-control d-inline float-end me-2" style="height:26px; width:170px; padding-left:10px; padding-top:2px;" name="commondity_id">
              <option value="">View Each Commodity</option>
              <?php
              $searchcommonditystmt = $pdo->prepare("SELECT DISTINCT commondity_id FROM hhkmcstock WHERE country = :country
                                                ...
                                                ");
              $searchcommonditystmt->execute([':country' => $country]);
              ...
              foreach ($searchcommonditydatas as $searchcommonditydata) {
                ...
                $item_name = $allProducts[$item_id] ?? 'Unknown';
              ?>
                <option value="<?php echo htmlspecialchars($item_id); ?>"><?php echo htmlspecialchars($item_name); ?></option>
              <?php
              }
              ?>
            </select>
        ...
        <div class="card-body">
          <form action="" method="post" class="text-center">
            ...
            ?>
              <button type="submit" class="pb-2 pt-2 ps-4 pe-4 text-dark rounded <?php echo htmlspecialchars($countrydata['country']); ?>link" style="text-decoration:none; border:none;" name="<?php echo htmlspecialchars($btnname); ?>"><?php echo htmlspecialchars($countrydata['country']) . " Stock"; ?></button>
            <?php
            }
            ?>
          </form>
          ...
          ?>
            <table class="table table-hover table-bordered table-striped hide" id="<?php echo htmlspecialchars($countrydata['country']); ?>table">
              ...
              if (isset($_POST['commonditybtn']) && !empty($_POST['commondity_id'])) {
                ...
                $hhkmcstockcommonditystmt = $pdo->prepare("SELECT DISTINCT commondity_id 
                                                          FROM hhkmcstock 
                                                          WHERE commondity_id = :commondity_id AND fish_type = :fish_type AND country = :country
                                                          UNION
                                                          SELECT DISTINCT commondity_id 
                                                          FROM gfcmcstock 
                                                          WHERE commondity_id = :commondity_id AND fish_type = :fish_type AND country = :country
                                                        ");
                $hhkmcstockcommonditystmt->execute([':commondity_id' => $searchcommondity, ':fish_type' => $searchfish_type, ':country' => $country]);
                ...
                SELECT DISTINCT commondity_id FROM gfcmcstock WHERE country = :country AND date BETWEEN :datefrom AND :dateto
                                                        ");
                $hhkmcstockcommonditystmt->execute([':country' => $country, ':datefrom' => $datefrom, ':dateto' => $dateto]);
                $hhkmcstockcommonditydatas = $hhkmcstockcommonditystmt->rowCount();
              } else {
                ...
                SELECT DISTINCT commondity_id FROM gfcmcstock WHERE country = :country
                ");
                $hhkmcstockcommonditystmt->execute([':country' => $country]);
                $hhkmcstockcommonditydatas = $hhkmcstockcommonditystmt->rowCount();
              }
              ...
              for ($i = 0; $i < $hhkmcstockcommonditydatas; $i++) {
                ...
                $commonditystmt->execute([':country' => $country]);
                ...
                if (isset($_POST['commonditybtn']) && !empty($_POST['commondity_id'])) {
                  ...
                  $searchstmt = $pdo->prepare("SELECT 
                                      ...
                                      GROUP BY 
                                          ...
                                          fish_type;
                                              ");
                  $searchstmt->execute([':commondity_id' => $searchcommondity_id, ':country' => $country, ':fish_type' => $searchfish_type]);
                  ...
                  $stmt = $pdo->prepare("SELECT 
                                      ...
                                      GROUP BY 
                                          ...
                                          fish_type;
                                      ");
                  $stmt->execute([':commondity_id' => $commondity_id, ':country' => $country, ':datefrom' => $datefrom, ':dateto' => $dateto]);
                  $datas = $stmt->fetchall();
                } else {
                  $stmt = $pdo->prepare("SELECT 
                                          id, 
                                      ...
                                      GROUP BY 
                                          ...
                                          fish_type;
                                      ");
                  $stmt->execute([':commondity_id' => $commondity_id, ':country' => $country]);
                  $datas = $stmt->fetchall();
                }
                ...
                foreach ($datas as $hhkdata) {
                  ...
                  $current_country = $hhkdata['country'];
                  ...
                  $fetchallstmt->execute([':size' => $size, ':item_id' => $item_id, ':kg' => $kg, ':country' => $current_country, ':fish_type' => $fish_type]);
                  ...
                  $fetchallgfcstmt->execute([':size' => $size, ':item_id' => $item_id, ':kg' => $kg, ':country' => $current_country, ':fish_type' => $fish_type]);
              ...
              ?>
                  <tr style="text-align:center !important; <?php if (empty($fetchalldata['balance_mc']) && empty($fetchallgfcdata['balance_mc'])) {
                                                              echo "display: none !important;";
                                                            } ?>">
                    <td><?php echo $id; ?></td>
                    <td><?php echo htmlspecialchars($item_name) . "(" . htmlspecialchars($hhkdata['fish_type']) . ")"; ?></td>
                    <td><?php echo htmlspecialchars($current_country); ?></td>
                ...
                <tr class="text-center" style="background-color:#c1f5cf;">
                  ...
                  <td style="font-weight: bold;"></td>
                  <td style="font-weight: bold;"></td>
                  <td style="font-weight: bold;"><?php if ($totalhhkmc != 0) {
                                                    echo $totalhhkmc;
                                                  } else {
                                                    echo "-";
                                                  }; ?></td>
    ...
    foreach ($countrydatas as $countrydata) {
      if (isset($_SESSION['tabs']) && $_SESSION['tabs'] == $countrydata['country']) {
        ...
        if ($_SESSION['tabs'] == $countrydata['country']) {
          ...
          foreach ($countrydatas as $cdata) {
            echo 'document.querySelector("#' . $cdata['country'] . 'table").classList.add(\'hide\');';
            echo 'document.querySelector(".' . $cdata['country'] . 'link").classList.remove(\'color\');';
          }
