    ...
    <div class="contentcol" id="content">
      ...
      <div class="card">
        <div class="card-header bg-info text-light">
          <h3 class="d-inline">Receivable for Customer</h3>
        </div>
        <div class="card-body">
          <table class="mt-3 table table-bordered table-striped rounded">
            ...
            // Aggregating customer invoice balances from receivables ledger
            $stmt = $pdo->prepare("
                SELECT 
                    c.id AS customer_id,
                    c.name AS customer_name,
                    COALESCE(SUM(r.invoice_amount), 0) AS total_billed,
                    COALESCE(SUM(r.paid_amount), 0) AS total_paid,
                    COALESCE(SUM(r.balance), 0) AS balance_owed
                FROM contacts c
                INNER JOIN receivable r ON c.id = r.ac_code
            ...
            ?>
              <tr>
                ...
                <td><?= number_format($cust['total_paid'], 2); ?></td>
                <td><?= number_format($cust['balance_owed'], 2); ?></td>
                <td>
            ...
            <tr style="font-weight: bold;">
              ...
              <td><?= number_format($grand_owed, 2); ?></td>
              <td></td>
            </tr>
          </table>
        </div>
