        ...
        <div class="card mt-3">
          ...
          <div class="card-body">
            ...
            if(isset($_POST['addbutton'])){
              ...
              $message = $query->addpayable('payable', $paid_date, $paid_amount, $supplier_id, $voucher_no, $totalbalance);
            }
            ?>
            <?php
            if(!empty($message)){
            ...
            <?php
              if(!empty($_GET['pageno'])){
                ...
                $voucher_no = $_GET['voucher_no'];
                $payablestmt = $pdo->prepare("SELECT DISTINCT voucher_no FROM purchase WHERE voucher_no='$voucher_no'");
                $payablestmt->execute();
                ...
                $paystmt = $pdo->prepare("SELECT * FROM payable WHERE supplier_id='$supplier_id'");
                $paystmt->execute();
                $paydatas = $paystmt->fetchall();
              }
            ?>
            <table class="mt-5 table table-bordered table-striped rounded">
              ...
              $stmt->execute();
              $purchasedatasum = $stmt->fetch(PDO::FETCH_ASSOC);
               ?>
                <tr>
                  <td>Payable Supplier:<?php echo $supplier_name['supplier_name']; ?></td>
                  <td>Voucher No:<?php echo $_GET['voucher_no']; ?></td>
                  <td>Payable Amount:<?php echo $purchasedatasum['amount']; ?></td>
                  <td></td>
                  <td></td>
                  <td></td>
                  <td></td>
              ...
              <?php
              $i=1;
              foreach ($paydatas as $payabledata) {
                ...
                $supplier_name = $suppliernamestmt->fetch(PDO::FETCH_ASSOC);
              ?>
              <tr <?php if(empty($paydatas)){
                echo "style='display:none;'";
              } ?>>

                <td><?php //echo $supplier_name['supplier_name']; ?></td>
                <td><?php //echo $payabledata['voucher_no']; ?></td>
                <td><?php //echo $payableamount['balance']; ?></td>
                <td><?php echo $payabledata['paid_date']; ?></td>
                <td><?php echo $payabledata['paid_amount']; ?></td>
                <td><?php echo $payabledata['balance']; ?></td>
                <td>
  ...
  <div class="modal fade" id="addmodal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        $amount = $purchasedatasum[0]['amount'];
         ?>
        <form action="" method="post" autocomplete="off">
          <input type="hidden" name="supplier_id" value="<?php echo $supplier_id; ?>">
          <input type="hidden" name="balance" value="<?php echo $amount; ?>">
          <div class="modal-body">
