      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <table class="mt-3 table table-bordered table-striped rounded">
            <tr>
              <th>No.</th>
              <th>Coldstore Name</th>
              <th>Action</th>
            </tr>

            <?php
            $stmt = $pdo->prepare("SELECT * FROM config_coldstore ORDER BY id");
            $stmt->execute();
            $rawResult = $stmt->fetchAll();
            $total_pages = ceil(count($rawResult) / $numOfrecs);

            $stmt = $pdo->prepare("SELECT * FROM config_coldstore ORDER BY id LIMIT $offset,$numOfrecs ");
            $stmt->execute();
                  ...
                  <div class="modal-content">
                    ...
                    <form action="" method="post" autocomplete="off">
                      <div class="modal-body">
                        <?php
                        $id = $itemdata['id'];
                        $updatedata = $query->select('config_coldstore', $id, 'id');
                        ?>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content" style="margin-top:100px;">
        <div class="modal-header bg-primary text-light">
          <h5 class="modal-title" id="addmodellabel">Create Coldstore</h5>
          <button type="button" class="btn" data-bs-dismiss="modal" aria-label="Close">
            <span aria-hidden="true" class="h3">&times;</span>
          </button>
        </div>
        <form action="configuration_coldstore.php" method="post" autocomplete="off">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-success" name="addbutton">Add Coldstore</button>
          </div>
        </form>
      </div>
    </div>
