      ...
      <div class="card">
        ...
        <div class="card-body">
          <table class="table table-striped table-hover table-hover">
            ...
            if (!empty($packingdatas) && is_array($packingdatas)) {
              foreach ($packingdatas  as $packingdata) {
                $customer_id = $packingdata['customer_id'];
                // FIXED: Fetch from unified contacts table
            ...
            ?>
                  ...
                  <td>
                    <div class="row">
                      ...
                      <div class="col-3 tooltip1">
                        <a href="packing_stock_info.php?infoid=<?php echo $packingdata['id']; ?>" class="btn btn-info btn-sm text-light"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-list-check" viewBox="0 0 16 16">
                      ...
                      <div class="col-3 tooltip3">
                        <a href="packingmaterial.php?infoid=<?php echo $packingdata['id']; ?>" class="btn btn-primary btn-sm text-light"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-seam-fill" viewBox="0 0 16 16">
                      ...
                      <div class="col-3 tooltip2">
                        <a href="invoice_costing.php?infoid=<?php echo $packingdata['id']; ?>" class="btn btn-secondary btn-sm text-light"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-currency-dollar" viewBox="0 0 16 16">
                          ...
                          <div class="row">
                            ...
                            <div class="col">
                              ...
                              <select class="form-control inpv2 mb-2" name="customer_idup">
                                <?php
                                // FIXED: Pull from contacts where they are flagged as a customer
                                $customerstmt = $pdo->prepare("SELECT * FROM contacts WHERE is_customer = 1");
                                $customerstmt->execute();
                                $customerdatas = $customerstmt->fetchAll(PDO::FETCH_ASSOC);
                                foreach ($customerdatas as $customerdata) {
                                ?>
                                  <option value="<?php echo $customerdata['id']; ?>" <?php if ($customerdata['id'] == $packingdata['customer_id']) {
                                                                                        echo 'selected';
                                                                                      } ?>><?php echo htmlspecialchars($customerdata['name']); ?></option>
                                <?php
                                }
                                ?>
                              </select>
              ...
              ?>
              <tr>
                <td colspan="6" class="text-center text-muted fw-bold py-4">No packing list data found.</td>
              </tr>
            <?php
            }
            ?>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 650px !important; margin-top:70px !important;">
              <div class="modal-header bg-info text-light">
                <h1 class="modal-title fs-5">Add Packing Data</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                <form action="packing_stock.php" method="post">
                  <div class="row">
                    ...
                    <div class="col">
                      ...
                      <select class="form-control inpv2 mb-2" name="customer_id" required>
                        <option value="">Select Customer...</option>
                        <?php
                        // Fetch contacts for the ADD modal (no $packingdata check here!)
                        $customerstmt = $pdo->prepare("SELECT * FROM contacts WHERE is_customer = 1");
                        $customerstmt->execute();
                        $customerdatas = $customerstmt->fetchAll(PDO::FETCH_ASSOC);
                        foreach ($customerdatas as $customerdata) {
                        ?>
                          <option value="<?php echo $customerdata['id']; ?>"><?php echo htmlspecialchars($customerdata['name']); ?></option>
                        <?php
                        }
                        ?>
                      </select>
