          ...
          <table class="table table-hover table-striped table-bordered" style="font-size: 13px;">
            ...
            try {
              ...
              if ($isViewMode) {
                if (!empty($_SESSION['search_tcl']['commondity'])) {
                  $conditions[] = "item_id = " . $pdo->quote($_SESSION['search_tcl']['commondity']);
                }
                if (!empty($_SESSION['search_tcl']['searchdate'])) {
                  $conditions[] = "date = " . $pdo->quote($_SESSION['search_tcl']['searchdate']);
                }
              ...
              echo '<tr><td colspan="23" class="text-center text-danger fw-bold">Query Error: ' . htmlspecialchars($e->getMessage()) . '</td></tr>';
            }
            ...
            try {
              if ($isViewMode && !empty($_SESSION['search_tcl']['commondity']) && !empty($_SESSION['search_tcl']['searchdate'])) {
                ...
                if (round($totalf7kgdata['total_kg'] ?? 0) != 0) {
            ...
            <?php if (count($datas) > 0) { ?>
              <tr style="background-color: #f8f9fa;">
                ...
                <td></td>
                <?php if ($isViewMode) { ?>
                  <td style="font-weight:bold; <?php if (strpos(round(floatval($percentage), 2), '-') !== false) echo 'color:red;'; ?>">
                    <?php echo ($percentage !== "" && $percentage !== "Error") ? round($percentage, 2) . "%" : "-"; ?>
                  </td>
                <?php } ?>
              </tr>
            <?php } ?>
